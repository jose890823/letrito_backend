...
export class SubscriptionsService {
  ...
  async createCheckoutSession(dto: CreateCheckoutSessionDto): Promise<{
    ...
    lineItems.push({
      price_data: {
        currency: 'usd',
        product_data: {
          name: `Letrito - Pago Inicial`,
          description: `Pago inicial para suscripción premium`,
        },
  ...
  private async handleCheckoutCompleted(
    ...
    // Registrar evento para el dashboard
  ...
  /**
   * Crear checkout session para Letrito
   * @param plan - Plan a suscribir (CREATOR $7/mes o PRO $15/mes)
   */
  async createLetritoCheckout(
    userId: string,
    successUrl: string,
    cancelUrl: string,
    ...
    this.logger.log(`Letrito checkout created: ${session.id} for plan ${plan}`);
