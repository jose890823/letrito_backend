...
export class UsersService {
  ...
  async getOnboardingStatus(
    ...
    const childProfilesCount = await this.childProfileRepository.count({
  ...
  /**
   * Complete onboarding - set timezone
   */
  async completeOnboarding(
    ...
    user.timezone = dto.timezone || 'America/Mexico_City';
    user.onboardingCompleted = true;

    await this.userRepository.save(user);

    this.logger.log(`Onboarding completado para usuario ${userId}`);
  ...
  async updateTimezone(userId: string, timezone: string): Promise<void> {
    ...
    this.logger.log(`Timezone actualizado para usuario ${userId}: ${timezone}`);
  }
