...
export class AuthService {
  ...
  async register(registerDto: RegisterDto) {
    ...
    const user = this.userRepository.create({
      ...
      lastName,
      phone,
      roles: [UserRole.CLIENT], // Array de roles
      otpCode,
      otpExpiresAt,
      otpAttempts: 0,
      emailVerified: false,
  ...
  async login(loginDto: LoginDto, ipAddress?: string, userAgent?: string) {
    ...
    await this.eventsService.track(user.id, EventType.USER_LOGIN, {
      ipAddress,
      userAgent,
    });
  ...
  async logout(userId: string, ipAddress?: string, userAgent?: string) {
    ...
    // Registrar evento para el dashboard
    await this.eventsService.track(user.id, EventType.USER_LOGOUT, {
      ipAddress,
      userAgent,
    });
  ...
  async forgotPassword(forgotPasswordDto: ForgotPasswordDto) {
    ...
    const frontendUrl =
      this.configService.get<string>('FRONTEND_URL') || 'https://letrito.app';
