...
export class User {
   ...
   */
  get role(): UserRole {
    if (!this.roles || this.roles.length === 0) return UserRole.CLIENT;
    // Orden de jerarquía: super_admin > admin > client
    const hierarchy = [UserRole.SUPER_ADMIN, UserRole.ADMIN, UserRole.CLIENT];
  ...
  // ============================================
  // RELACIÓN CON PERFILES DE NIÑOS (LETRITO)
  // ============================================
  ...
  @ApiProperty({
    example: false,
    description: 'Indica si el usuario completó el onboarding',
  })
  @Column({ type: 'boolean', default: false })
  ...
  @ApiProperty({
    ...
    description: 'Zona horaria del usuario para generación diaria',
    required: false,
  })
  @Column({ type: 'varchar', length: 100, nullable: true })
